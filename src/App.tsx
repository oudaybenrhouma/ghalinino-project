...
// Shown to logged-in users whose wholesale status is 'rejected'
// so they're not left wondering why their account changed.
// ============================================================================

function WholesaleStatusBanner({ user, language }: { user: any; language: 'ar' | 'fr' }) {
  ...
  return (
    <div className="bg-red-600 text-white px-4 py-2.5 flex items-center justify-between gap-4">
      <div className="flex items-center gap-3 min-w-0">
        <svg className="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
        </svg>
        <span className="text-sm font-medium truncate">
          {t('تم رفض طلب حساب تاجر الجملة الخاص بك.', 'Votre demande de compte grossiste a été refusée.')}
          {user?.wholesaleRejectionReason && (
            <span className="text-red-200 ml-1">
              {t('السبب:', 'Raison :')} {user.wholesaleRejectionReason}
            </span>
          )}
        </span>
      </div>
      <div className="flex items-center gap-3 shrink-0">
        <Link
          ...
          className="text-xs font-semibold bg-white text-red-600 px-3 py-1 rounded-full hover:bg-red-50 transition-colors whitespace-nowrap"
        >
          {t('إعادة التقديم', 'Faire une nouvelle demande')}
        </Link>
        <button onClick={() => setDismissed(true)} className="text-red-200 hover:text-white transition-colors" aria-label="Dismiss">
...
function HomePage() {
  ...
  return (
    ...
    >
      ...
      {/* ── WHOLESALE STATUS BANNER ─────────────────────────────────────────── */}
      {isAuthenticated && user && (user.wholesaleStatus === 'rejected') && (
        <WholesaleStatusBanner user={user} language={language} />
      )}
